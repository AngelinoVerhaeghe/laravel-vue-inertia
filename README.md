...
- **Blog search** — the homepage and Blog index hero each render a `BlogSearchForm` that submits a GET request to `/blog?q=…` (the same URL the homepage's `WebSite` JSON-LD `SearchAction` advertises). `BlogIndexController` sanitizes `q` (trim + 100-char cap) and applies a case-insensitive `LIKE` filter across `title`, `excerpt`, and `body_markdown`. The filter is preserved across pagination links; search-result pages get a search-aware title, canonical (with `?q=`), `noindex`, a results banner with a "Clear search" link, and a query-aware empty state.
- **Homepage pillars + pinned headline** — between the hero and the Featured grid, the homepage renders a *What you'll find here* strip with three curated pillar cards (Frontend / Backend / Web) that link into the blog search (`/blog?q=…`), followed by an *Editor's pick* hero card. The card is backed by an `is_headline` Filament toggle on `BlogPost`: toggling a post on automatically unpins any other post (single-pinned invariant enforced via the model's `saving` hook). When nothing is pinned, the card falls back to the most recently published post, so the slot is never empty as long as the blog has any published content.
